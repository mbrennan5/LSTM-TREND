...
#   3. Bayesian Optimization (GP-minimise) over the (indicator_n × z_n) joint space
#      BO converges ~20-40% faster than grid/GA — only ~20-30 evaluations needed
#   4. Final sovereign audit at the optimal pair (Block 6 ledger + Block 9 charts)
#
...
#                BO searches for the optimal z_n jointly with indicator_n.
#
...
def _process_symbol_worker(args):
    ...
    try:
        ...
        if len(raw_df) < max(120, indicator_n * 4):
            return None
        proc = generate_factory_features_v2(raw_df, indicator_n, z_n, selected_features)
        if proc.empty:
            return None
        proc['symbol'] = symbol
...
def load_hybrid_data_parallel(brain_name: str,
    ...
    items = [(sym, df.to_dict(), indicator_n, z_n, selected_features)
...
# ==============================================================================
# BLOCK 5: GPU-ACCELERATED AUDIT  (Ron Harper 4.30 — now returns baseline metric)
# ==============================================================================
...
def run_judicial_audit(brain_name, master_df, model_type='GRU',
    ...
    baseline_acc  : scalar validation accuracy  ← used as BO objective
    """
...
# ==============================================================================
# JOINT OPTIMISER — Bayesian Optimisation over (indicator_n × z_n)
#
# Why BO over grid search or GA?
#   • BO requires only ~20-30 evaluations vs 100-1000 for GA/grid
#   • Converges 20-40% faster; achieves ~95% optimality vs ~80-90% for GA
#   • Maintains a Gaussian Process surrogate → intelligently proposes next point
#   • Acquisition function (Expected Improvement) balances exploration/exploitation
# ==============================================================================
def _bo_objective(params,
    ...
    print(f"  [BO] ind_n={ind_n:3d} | z_n={z_n:3d} | acc={acc:.4f}")
    return 1.0 - acc            # minimise → maximise accuracy


def run_lookback_tester():
    print("\n" + "═"*68)
    print("  TA Feature Factory — Joint Lookback × Z-Lens Optimizer")
    print("  Engine: Bayesian Optimisation (Gaussian Process)")
